...
REF_MARKERS = {"OmecamtivMecarbil": "*", "Mavacamten": "^", "Aficamten": "D"}        #change show settinghere
REF_SIZE = 100
...
def _plot_umap(em, color_vals, ref_ems, active_refs, title, odir, fname):
...
def gen_umap_fp(ind: str, odir: str, jn: str, ref_ind: str = None, ref_keys=None):
    ...
    X = np.stack(fpdfc["fp"])
    ma = umap.UMAP(n_neighbors=15, min_dist=0.1, metric="jaccard", random_state=42)
    em = ma.fit_transform(X)
    ...
    if ref_ind is not None and active_refs:
        ...
        for name in active_refs:
            ...
            fp = fpgen(canon)
            ref_ems[name] = ma.transform(np.array([fp]))
            print(f"{name}: {ref_ems[name]}")

    _plot_umap(em, fpdfc[f"{jn}_r_i_docking_score"], ref_ems, active_refs,
               f"UMAP (FP) - {jn} Docking Score (w/ ref)",
               odir, f"{base}_{jn}_umap_fp_score_{timestamp}.png")

    _plot_umap(em, fpdfc["step"], ref_ems, active_refs,
               "UMAP (FP) - Step (w/ ref)",
               odir, f"{base}_{jn}_umap_fp_step_{timestamp}.png")

def _umap_desc_core(dfc, jn, odir, base, timestamp, desc_cols, label, active_refs, ref_desc_df=None):
    X = dfc[desc_cols].values
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)

    ma = umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean", random_state=42)
    em = ma.fit_transform(X_scaled)
    ...
    if ref_desc_df is not None and active_refs:
        ...
        ref_em = ma.transform(ref_X_scaled)
        ref_ems = {name: ref_em[[i]] for i, name in enumerate(active_refs)}
        for name in active_refs:
            print(f"{name}: {ref_ems[name]}")

    _plot_umap(em, dfc[f"{jn}_r_i_docking_score"], ref_ems, active_refs,
               f"UMAP ({label}) - {jn} Docking Score (w/ ref)",
               odir, f"{base}_{jn}_umap_{label}_score_{timestamp}.png")

    _plot_umap(em, dfc["step"], ref_ems, active_refs,
               f"UMAP ({label}) - Step (w/ ref)",
               odir, f"{base}_{jn}_umap_{label}_step_{timestamp}.png")

def gen_umap_desc(ind: str, odir: str, jn: str, ref_ind: str = None, mode: str = "both", ref_keys=None):
    base = os.path.splitext(os.path.basename(ind))[0]
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    active_refs = resolve_refs(ref_keys)
    ...
    if mode in ("all", "both"):
        _umap_desc_core(dfc, jn, odir, base, timestamp, DESC_COLS_ALL, "all_desc", active_refs, ref_desc_df)
    if mode in ("select", "both"):
        _umap_desc_core(dfc, jn, odir, base, timestamp, DESC_COLS_SELECT, "sel_desc", active_refs, ref_desc_df)

def run_umap(ind: str, odir: str, jn: str, ref_ind: str = None, mode: str = "both",
             ref_keys=None, feat: str = "both"):
    """feat: 'fp', 'desc', or 'both' -- selects which UMAP variant(s) to run."""
    if feat in ("fp", "both"):
        gen_umap_fp(ind, odir, jn, ref_ind, ref_keys)
    if feat in ("desc", "both"):
        gen_umap_desc(ind, odir, jn, ref_ind, mode, ref_keys)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate UMAP plots. Please check jn !!!!!! Default set to PPS")
    parser.add_argument("--ind", type=str, required=True, help="Input [_CSV_] path")
    ...
    parser.add_argument("--feat", type=str, default="both", choices=["fp", "desc", "both"],
                        help="Which UMAP variant(s) to run: fp, desc, or both (default: both)")
    args = parser.parse_args()

    run_umap(args.ind, args.odir, args.jn, args.ref_ind, args.mode, args.ref, args.feat)


# python 1_1_UMAP.py --ind /home/andy/proj/701/code/developing_dir/tempdevtest/scoresPPS_cleaned.csv --odir /home/andy/proj/701/code/developing_dir/tempdevtest/figs --ref OM --feat fp