...
When `reference_interaction_analysis=on`, the completed-analysis runner also
runs ProLIF for the bundled PPS and PR reference complexes and creates
`work_results/analysis/reference_interactions/<run_id>/reference_interactions.csv`.
This consolidated CSV is generated by the pipeline; users do not prepare it
manually. The hotspot figure configurations consume that file together with
the six selected Fpocket `pocket*_atm.pdb` files.
