...
def parse_config(path):
    ...
    cfg["posecheck_workers"] = int(
        cfg.get("posecheck_workers", "").strip() or "30"
    )
    chunk_size = (
        cfg.get("posecheck_chunk_size", "").strip().lower() or "auto"
    )
    cfg["posecheck_chunk_size"] = (
        "auto" if chunk_size == "auto" else int(chunk_size)
    )
    ...
    cfg["prolif_workers"] = int(prolif_workers) if prolif_workers else 30
