...
def run_single_complex(
    ...
    posecheck_workers=1,
    posecheck_chunk_size="auto",
    prolif_workers=None,
    resume=False,
    _include_elapsed=True,
):
    ...
    if analysis in {"posecheck", "both"}:
        ...
        if int(posecheck_workers) > 1:
            resolved_chunk_size = (
                max(1, math.ceil(len(metadata) / int(posecheck_workers)))
                if str(posecheck_chunk_size).strip().lower() == "auto"
                else int(posecheck_chunk_size)
            )
        ...
        status = ("posecheck", "completed", len(quality))
        statuses.append(status + ((time.monotonic() - component_started,) if _include_elapsed else ()))
    ...
    if analysis in {"prolif", "both"}:
        ...
        status = ("prolif", "completed", len(interactions))
        statuses.append(status + ((time.monotonic() - component_started,) if _include_elapsed else ()))
        print(
    ...
    columns = ["analysis_component", "status", "output_rows"]
    if _include_elapsed:
        columns.append("elapsed_seconds")
    summary = pd.DataFrame(statuses, columns=columns)
...
def main(argv=None, *, include_elapsed=True):
    ...
    run_single_complex(
        ...
        resume=args.resume,
        _include_elapsed=include_elapsed,
    )
