...
def parse_config(path: Path) -> dict[str, str]:
    ...
    config.setdefault(
        "bottom_group_title",
        "Own vs opposite receptor-state docking-score distributions",
    )
    config.setdefault("bottom_group_title_font_size", "12")
    config.setdefault("bottom_group_title_pad", "0.035")
    return config
...
def plot_density(results_dir: Path, output_dir: Path, input_mode: str,
    ...
    if input_mode == "own-state":
        ...
        if config["bottom_group_title"]:
            fig.canvas.draw()
            bottom_group_y = (
                max(ax.get_position().y1 for ax in bottom_axes)
                + float(config["bottom_group_title_pad"])
            )
            fig.text(
                0.5,
                bottom_group_y,
                config["bottom_group_title"],
                ha="center",
                va="bottom",
                fontsize=float(config["bottom_group_title_font_size"]),
                fontweight="normal",
            )
    else:
    ...
    output_paths = allocate_versioned_group(
        output_dir, (f"{stem}.png", f"{stem}.svg")
    )
    for output_path in output_paths:
        fig.savefig(output_path, dpi=int(config["dpi"]), bbox_inches="tight")
    plt.close(fig)
