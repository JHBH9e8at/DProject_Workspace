...
def draw_figure(
    ...
    top, row_h, bottom = 185 * scale, 42 * scale, 105 * scale
    ...
    draw.text((width / 2, 28 * scale), "PR/PPS residue interaction hotspots: full population vs last 50 iterations", fill=color["text"], font=title, anchor="ma")
    draw.text(((left_outer + left_inner) / 2, 112 * scale), "PPS prevalence", fill=color["PPS_last"], font=bold, anchor="ma")
    draw.text(((backbone_left + backbone_right) / 2, 112 * scale), "Residue", fill=color["text"], font=bold, anchor="ma")
    draw.text(((right_inner + right_outer) / 2, 112 * scale), "PR prevalence", fill=color["PR_last"], font=bold, anchor="ma")

    legend_y = 78 * scale
    legend = [
        ("Full population", color["PPS_full"], 500), ("Last 50", color["PPS_last"], 595),
        ("Full population", color["PR_full"], 1120), ("Last 50", color["PR_last"], 1215),
    ]
    for label, fill, xpos in legend:
        x = xpos * scale
        draw.rectangle((x, legend_y, x + 11 * scale, legend_y + 11 * scale), fill=fill)
        draw.text((x + 15 * scale, legend_y + 5 * scale), label, fill=color["muted"], font=small, anchor="lm")
    ...
    for x, label in (
        ...
        draw.text((x, 148 * scale), label, fill=color["muted"], font=small, anchor="mm")
    ...
    for row, residue in enumerate(residues):
        ...
        draw.line(
            (backbone_left, row_top, backbone_right, row_top),
            fill="#EFEFEF",
            width=1 * scale,
        )
        ...
        for state, px in (("PPS", pps_pocket_x), ("PR", pr_pocket_x)):
            ...
            if len(hits) == 1:
                ...
                label = "-" if not hits else "/".join(item.split()[-1].removeprefix("P") for item in hits)
    ...
    draw.line(
        (backbone_left, top + len(residues) * row_h, backbone_right, top + len(residues) * row_h),
        fill="#EFEFEF",
        width=1 * scale,
    )
    ...
    image.save(OUTPUT_DIR / f"{OUTPUT_STEM}.png", dpi=(300, 300))
