...
def calculate_embedding(matrix, method, random_state):
    if method == "umap":
        import umap
        reducer = umap.UMAP(
            n_neighbors=15, min_dist=0.1,
            metric="jaccard" if matrix.dtype == np.uint8 else "euclidean",
            random_state=random_state,
        )
    else:
        reducer = TSNE(
...
def build_one(df, paths, cache_dir, method, feature, random_state, cache_mode):
    ...
    if feature == "fp":
        selected = df.copy()
        matrix = fingerprint_matrix(selected["canon_smiles"].tolist())
    else:
        ...
        matrix = StandardScaler().fit_transform(selected[cols].astype(float))

    print(f"[calculate] {method}/{feature}: {matrix.shape}")
    embedding = calculate_embedding(matrix, method, random_state)
    result = selected[
        [
