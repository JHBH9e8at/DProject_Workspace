...
def pairwise_similarity(fps):
    ...
    similarities = []
    nearest = np.zeros(len(fps), dtype=float)
    for index in range(1, len(fps)):
        current = np.asarray(
            DataStructs.BulkTanimotoSimilarity(fps[index], fps[:index]),
            dtype=float,
        )
        similarities.extend(current.tolist())
        nearest[index] = current.max()
    ...
    return {
        ...
        "fraction_nearest_ge_0_8_within_scope": float((nearest >= 0.8).mean()),
    }
