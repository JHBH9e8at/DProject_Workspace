...
def load_refs(ref_keys, ref_csv=None):
    if not ref_keys:
        return {}
    ref_df = pd.read_csv(ref_csv or DEFAULT_REF_CSV)
    refs = {}
    for key in ref_keys:
        name = REF_MAP[key]
...
def mainr(ind, odir, jn, threshold, ref_keys, ref_csv=None):
    print(f"Loading {ind}")
    df = pd.read_csv(ind)
    refs = load_refs(ref_keys, ref_csv)
